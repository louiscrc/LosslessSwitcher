...
class Defaults: ObservableObject {
    ...
    private let kFallbackSampleRateKHz = "com.vincent-neo.LosslessSwitcher-Key-FallbackSampleRateKHz"
    
    static let fallbackSampleRateOptionsKHz: [Double] = [44.1, 48, 88.2, 96]
    ...
    private init() {
        ...
        self.fallbackSampleRateKHz = UserDefaults.standard.double(forKey: kFallbackSampleRateKHz)
    }
    ...
    @Published var fallbackSampleRateKHz: Double {
        willSet {
            UserDefaults.standard.set(newValue, forKey: kFallbackSampleRateKHz)
        }
    }
    
    var fallbackSampleRateHz: Float64 {
        fallbackSampleRateKHz * 1000
    }
    
    func isFallbackSampleRate(_ rateKHz: Double) -> Bool {
        abs(fallbackSampleRateKHz - rateKHz) < 0.05
    }
